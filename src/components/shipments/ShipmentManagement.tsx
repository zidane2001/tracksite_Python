...
export const ShipmentManagement: React.FC = () => {
  ...
  const handleConfirmShipment = async (id: number) => {
    const totalFreight = prompt('Entrez les frais de port (€):');
    const expectedDelivery = prompt('Date de livraison estimée (YYYY-MM-DD):');
    const comments = prompt('Commentaires supplémentaires:');

    if (totalFreight && expectedDelivery) {
      try {
        const response = await fetch(`http://localhost:3005/api/shipments/${id}/confirm`, {
          method: 'POST',
        ...
        if (response.ok) {
          ...
          setNotification({type: 'success', message: `Expédition confirmée avec succès. Numéro de suivi: ${data.tracking_number}`});
  ...
  const handleCreateShipment = async () => {
    ...
    const tracking = `SHIP${Math.floor(100000000000 + Math.random() * 899999999999)}-COLISSELECT`;
  ...
  return <div>
    ...
    <div className="bg-white rounded-lg shadow overflow-hidden">
          ...
          <tbody>
            {paginatedShipments.map(shipment => <tr key={shipment.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 border-b text-right">
                ...
                <button className="text-purple-600 hover:text-purple-800 p-1 ml-2" title="Tracking History" onClick={() => openTrackingModal(shipment)}>
                  <HistoryIcon size={16} />
                </button>
                ...
                {shipment.status === 'pending_confirmation' && (
                  <>
                    <button className="text-green-600 hover:text-green-800 p-1 ml-2" title="Accepter" onClick={() => handleConfirmShipment(shipment.id)}>
                      ✓ Accepter
                    </button>
                    <button className="text-red-600 hover:text-red-800 p-1 ml-2" title="Rejeter" onClick={() => handleRejectShipment(shipment.id)}>
                      ✗ Rejeter
                    </button>
                  </>
                )}
      ...
      {isAddOpen && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
          <div className="p-4 md:p-6">
            ...
            <div className="flex flex-col sm:flex-row justify-end gap-2 mt-4 md:mt-6">
              <button className="px-4 py-2 border rounded-lg text-sm md:text-base order-2 sm:order-1" onClick={() => setIsAddOpen(false)}>Annuler</button>
              <button className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm md:text-base order-1 sm:order-2" onClick={handleCreateShipment}>Créer le colis</button>
            </div>
          </div>
        </div>
      </div>}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="p-4 md:p-6">
              ...
              <div className="flex flex-col sm:flex-row justify-end gap-2 mt-4 md:mt-6">
                <button
                  className="px-4 py-2 border rounded-lg text-sm md:text-base order-2 sm:order-1"
                  onClick={() => {
                    setIsEditOpen(false);
                    setEditingShipment(null);
                  }}
                ...
                <button
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm md:text-base order-1 sm:order-2"
          ...
          <div className="bg-white rounded-lg shadow-lg w-full max-w-6xl max-h-[90vh] overflow-hidden">
            <div className="p-6 border-b">
              <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold">Tracking History - {selectedShipment.tracking_number}</h2>
                <button
